...
from todoist_project_utils import add_todoist_task, resolve_project_id


def send_whatsapp_reply(to_number, message):
    client = Client(os.environ["TWILIO_ACCOUNT_SID"],
                    os.environ["TWILIO_AUTH_TOKEN"])
    from_number = "whatsapp:" + os.environ["TWILIO_WHATSAPP_NUMBER"]
...
- For reminders, only fill:
  ...
  - datetime_string = if any vague or exact time is mentioned (e.g. 'מחר ב-10', 'ביום ראשון בבוקר'), translate it into natural English for Todoist (e.g. 'tomorrow at 10am')
  - project_name = name of the Todoist project/list this reminder should be added to (e.g. 'מוזיקה', 'אישית') — only include this if the message mentions a specific list
...
- If the message is a calendar query (e.g., 'מה יש לי מחר', 'מתי ההופעה הבאה שלי'), return:
  {
    "type": "query",
    "intent": "next_event" | "day_summary" | "has_event",
    "target_day": "a real date in YYYY-MM-DD format (e.g. 2025-05-15)" (if relevant),
    "search_keywords": "string" (optional, e.g. 'חזרה', 'הופעה')
  }
    ...
    try:
        ...
        if parsed["type"] == "query":
            ...
            try:
                ...
                target_day = start_dt.strftime("%Y-%m-%d")
            ...
            if intent == "day_summary":
                if not events:
                    answer = "אין לך כלום ביום הזה 📭"
                else:
                    answer = "ביום הזה יש לך:\n" + "\n".join([
                        f"🗓 {e['summary']} ב־{e['start'].get('dateTime', '')[11:16]}"
                        for e in events
                    ])

            elif intent == "has_event":
                found = [e for e in events if keyword in e["summary"]]
                answer = f"✅ כן, יש לך {keyword}" if found else f"❌ לא מצאתי {keyword} ביום הזה"

            elif intent == "next_event":
                future_events = calendar_service.events().list(
                    calendarId='primary',
                ...
                if future_events:
                    e = future_events[0]
                    time_str = e['start'].get('dateTime', '')[11:16]
                    date_str = e['start'].get('dateTime', '')[:10]
                    answer = f"האירוע הבא שלך הוא {e['summary']} בתאריך {date_str} בשעה {time_str}"
                else:
                    answer = "לא מצאתי אירועים קרובים ביומן שלך."

            send_whatsapp_reply(request.form.get("From"), answer)
        ...
        if parsed["type"] == "reminder":
            ...
            if "project_name" in parsed and parsed["project_name"]:
                project_id = resolve_project_id(parsed["project_name"])

            result = add_todoist_task(task_text, due_string=due, project_id=project_id)
