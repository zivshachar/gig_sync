...
def webhook():
	...
	try:
		...
		if parsed["action"] == "add_calendar_event":
			...
			reply_text = parsed["reply_template"]
				...
				print("📩 Incoming raw date from GPT:", raw_date)

				resolved_date = resolve_natural_date(raw_date)
				print("📅 Resolved actual date:", resolved_date)
	...
	try:
		start_time = first_event["start"].get("dateTime")
		if start_time:
	...
	reply_text = parsed["reply_template"].format(title=title, time=time)
