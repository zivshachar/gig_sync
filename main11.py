...
try:
    ...
    from drive_utils import (build_drive_service, upload_file,
                             list_drive_files, delete_file, download_file,
                             export_google_doc)
...
class WhatsAppAssistant:
        ...
        self.function_descriptions = [{
            ...
            "parameters": {
                ...
                "properties": {
                    ...
                    "due_string": {
                        "type": "string",
                        "description": "Due date as string (optional)"
                    },
                    "project_id": {
                        "type": "string",
            ...
            "parameters": {
                ...
                "properties": {
                    "calendar_service": {
                        ...
                        "string",
                        "description":
                        "Calendar service (will be built automatically)"
                    },
                    "calendar_id": {
                        "type": "string",
        ...
        try:
            response = openai.ChatCompletion.create(
                ...
                messages=[{
                    ...
                    "system",
                    "content":
                    """You are a helpful WhatsApp assistant that can manage tasks, calendar events, and documents. 
    ...
    def execute_function(self, function_name, arguments):
        ...
        try:
            ...
            if function_name in [
                    'fetch_income_summary', 'get_gig_history', 'log_event',
                    'append_row'
            ]:
                try:
            ...
            if function_name in [
                    'list_drive_files', 'upload_file', 'download_file',
                    'delete_file', 'export_google_doc'
            ]:
                try:
                    result = func(**arguments) if arguments else func()
                    return result
            ...
            logger.info(
                f"Function {function_name} expects parameters: {list(sig.parameters.keys())}"
            )
            ...
            if message.get("function_call"):
                ...
                final_response = openai.ChatCompletion.create(
                    ...
                    messages=[{
                        ...
                        "system",
                        "content":
                        "You are a helpful WhatsApp assistant. Provide a concise, friendly response based on the function result."
                    }, {
                        "role": "user",
                        "content": user_message
                    }, message, {
    ...
    <body>
        <div class="container">
            ...
            <div class="status">
                ...
                <strong>🕐 Started:</strong> ''' + datetime.now().strftime(
        '%Y-%m-%d %H:%M:%S') + '''<br>
